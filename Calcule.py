...
def load_file1():
    global file_path1
    file_path1 = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx"), ("PDF files", "*.pdf"), ("Image files", "*.png;*.jpg;*.jpeg")])
    if file_path1:
...
# Fonction pour sauvegarder le fichier Excel mis à jour
...
label_instruction = tk.Label(frame, text="Sélectionnez les fichiers pour récupérer et mettre à jour les cotes", bg=bg_color, font=(font_family, 12))
label_instruction.pack(pady=10)

button_file1 = tk.Button(frame, text="Charger le premier fichier", command=load_file1, bg=button_color, fg=button_text_color, font=(font_family, 10))
...
button_file2 = tk.Button(frame, text="Charger le deuxième fichier", command=load_file2, bg=button_color, fg=button_text_color, font=(font_family, 10))
...
button_process = tk.Button(frame, text="Traiter les fichiers", command=process_files, bg=button_color, fg=button_text_color, font=(font_family, 10))
button_process.pack(pady=10, fill="x")

button_clear = tk.Button(frame, text="Vider les fichiers chargés", command=clear_files, bg=button_color, fg=button_text_color, font=(font_family, 10))
button_clear.pack(pady=10, fill="x")

# Section du tableau des matricules manquants
label_missing = tk.Label(root, text="Matricules non trouvés et données non copiées :", bg=bg_color, font=(font_family, 12))
label_missing.grid(row=2, column=0, padx=10, pady=10, sticky="ew")

treeview = ttk.Treeview(root, columns=("Matricule", "Valeur Ancienne"), show="headings", height=10)
...
style.configure("Treeview.Heading", font=(font_family, 11, "bold"), background=button_color, foreground=button_text_color)
